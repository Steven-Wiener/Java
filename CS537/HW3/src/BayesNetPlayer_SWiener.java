...
///////////////////////////////////////////////////////////////////////////////
//Title:            PlayNannon
//Files:            BayesNetPlayer_SWiener.java
...
public class BayesNetPlayer_SWiener extends NannonPlayer {
	// Defaults
	...
	int boardCreatePrimeWin[][] = new int[boardSize][3]; 
	int boardCreatePrimeLose[][] = new int[boardSize][3];

	@Override
	public String getPlayerName() { return "BayesNetPlayer_SWiener"; }
	
	// Constructors.
	public BayesNetPlayer_SWiener() {
		if (boardSize == 0) boardSize = 6;
		if (pieces == 0) pieces = 3;
		initialize(boardSize, pieces);
		
	}
	public BayesNetPlayer_SWiener(NannonGameBoard gameBoard) {
		super(gameBoard);
	...
	private void initialize(int cells, int pieces) {
		BayesNetPlayer_SWiener.boardSize = cells;
		BayesNetPlayer_SWiener.pieces = pieces;
		int homeWin[] = new int[pieces + 1]; // P(homeX | win)
		int homeLose[] = new int[pieces + 1]; // P(homeX | !win)
		...
		for (int i = 0; i < boardSize; i++)
			for (int j = 0; j < 3; j++) {
				winBoard[i][j] = 1; 
				loseBoard[i][j] = 1;
				boardCreatePrimeWin[i][j] = 1;
				boardCreatePrimeLose[i][j] = 1;
			}
	}
	...
	public List<Integer> chooseMove(int[] boardConfiguration, List<List<Integer>> legalMoves) {
		...
		if (legalMoves != null) for (List<Integer> move : legalMoves) { // <----- be sure to drop the "false &&" !
			...
			double pBrokeHitWin = 1;
			double pBrokeHitLose = 1;
			double pBoardWin = 1;
			double pBoardLose = 1;
    		...
    		if (hitOpponent & brokeMyPrime) {
    			pBrokeHitWin = (double) brokeHitWin / (double) wins;
    			pBrokeHitLose = (double) brokeHitLose / (double) losses;
    		}
	    	...
	    	// Assuming dependence between probability of break and a hit
	    	double winLikeliness = pHomeWin * pSafeWin * pBrokeHitWin * pExtendsPrimeOfMineWin * pCreatesPrimeOfMineWin * pBoardWin;
	    	double lossLikeliness = pHomeLose * pSafeLose * pBrokeHitLose * pExtendsPrimeOfMineLose * pCreatesPrimeOfMineLose * pBoardLose;
			...
			bestRatio = (pBest > bestRatio) ? pBest : bestRatio;
		}
	...
	public void updateStatistics(boolean             didIwinThisGame, 
		...
		for (int myMove = 0; myMove < numberOfMyMovesThisGame; myMove++) {
		    ...
		    if (didIwinThisGame) {
	    		...
	    		if (brokeMyPrime && hitOpponent) brokeHitWin++;
		    	...
		    	for (int i = 0; i < boardSize; i++) {
		    		winBoard[i][resultingBoard[i + 7]]++;
		    		if (createsPrimeOfMine) boardCreatePrimeWin[i][resultingBoard[i + 7]]++;
		    	}
		    } else {
		    	if (hitOpponent) hitOpponentLose++;
		    	if (brokeMyPrime) brokeMyPrimeLose++;
	    		if (brokeMyPrime && hitOpponent) brokeHitLose++;
		    	...
		    	for (int i = 0; i < boardSize; i++) {
		    		loseBoard[i][resultingBoard[i + 7]]++;
		    		if (createsPrimeOfMine) boardCreatePrimeLose[i][resultingBoard[i + 7]]++;
		    	}
		    }
			//}
		}
	...
	public void reportLearnedModel() {
		...
		Utils.println("Best home pieces value: " + Integer.toString(bestBreakAndHit));
